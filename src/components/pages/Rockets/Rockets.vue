<template>
  <v-container class="py-4">
    <div class="d-flex ga-2">
      ...
      <v-btn prepend-icon="mdi-plus" size="large" @click="onAdd">
        Add Data
      </v-btn>
    </div>
    ...
    <v-row class="mt-4" no-gutters>
      <v-col
        v-for="(item, id) in filteredRockets"
        :key="id"
        cols="6"
        sm="12"
        md="4"
      ...
      >
        <v-card class="ma-2" rounded="lg">
          <v-carousel
            ...
            cycle
          >
            <template v-if="item.flickr_images?.length">
              <v-carousel-item
                v-for="(image, idx) in item.flickr_images"
                :key="`${item.id}-${idx}`"
                :src="image"
                cover
              />
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              variant="text"
              :aria-label="item.show ? 'Hide details' : 'Show details'"
...
type RocketUI = Rocket & { show: boolean };
...
const norm = (value: string) => (value ? value.trim().toLowerCase() : "");

const filteredRockets = computed(() => {
  const keyword = norm(search.value);
  if (!keyword) return rockets.value;

  return rockets.value.filter((result) => {
    const name = norm(String(result.name ?? ""));
    const desc = norm(String(result.description ?? ""));
    return name.includes(keyword) || desc.includes(keyword);
  });
});

function toggleDetails(id: string) {
  const rocket = rockets.value.find((result) => result.id === id);
  if (rocket) rocket.show = !rocket.show;
}
